...
class ErrorBoundaryInner extends Component{
	...
	render() {
		const {hasError} = this.state;
		const {isLoading, reportStatus, setReportStatus, hasNavigation} = this.props;

		if(hasError){
				return (
				<div className='flex flex-col h-screen w-full items-center justify-center bg-slate-900'>
					<ErrorBlock
						hasControls={false}
						className=' mb-6'
